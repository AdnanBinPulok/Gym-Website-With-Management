...
@router.get("/pay-now", response_class=HTMLResponse, dependencies=[Depends(rate_limiter)])
async def pay_now_page(request: Request):
    try:
        ...
        logger.debug(f"Pay Now page data: {data}")
        ...
        logger.error(f"Error loading pay now page: {traceback.format_exc()}")
